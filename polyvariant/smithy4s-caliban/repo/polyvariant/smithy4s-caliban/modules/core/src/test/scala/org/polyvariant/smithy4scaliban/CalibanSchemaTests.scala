...
object CalibanSchemaTests extends SimpleIOSuite {
  ...
  test("list schema") {
    ...
    testQueryResultWithSchema(
      List("a", "b", "c"),
      "query { items }",
    )(using Schema.list(Schema.string).nested("items"))
  ...
  test("indexedSeq schema") {
    testQueryResultWithSchema(
      IndexedSeq("a", "b", "c"),
      "query { items }",
    )(using Schema.indexedSeq(Schema.string).nested("items"))
      .map(assert.eql(_, Json.obj("items" := List("a", "b", "c"))))
  }
  ...
  test("vector schema") {
    testQueryResultWithSchema(
      Vector("a", "b", "c"),
      "query { items }",
    )(using Schema.vector(Schema.string).nested("items"))
      .map(assert.eql(_, Json.obj("items" := List("a", "b", "c"))))
  }
  ...
  test("set schema") {
    testQueryResultWithSchema(
      Set("a", "b", "c"),
      "query { items }",
    )(using Schema.set(Schema.string).nested("items"))
      .map(assert.eql(_, Json.obj("items" := List("a", "b", "c"))))
  }
  ...
  test("enum schema") {
    ...
    )(using Ingredient.schema.nested("item"))
  ...
  test("int enum schema") {
    testQueryResultWithSchema(
      ...
      """query { item }""".stripMargin,
    )(using EnumResult.schema.nested("item"))
  ...
  test("blob schema") {
    testQueryResultWithSchema(
      ...
      """query { item }""".stripMargin,
    )(using Schema.bytes.nested("item"))
  ...
  test("document schema") {
    testQueryResultWithSchema(
      ...
      """query { item }""".stripMargin,
    )(using Schema.document.nested("item"))
      .map(
        assert.eql(
          _,
          Json.obj(
  ...
  test("timestamp schema (default: epoch second)") {
    testQueryResultWithSchema(
      Timestamp.epoch,
      """query { item }""".stripMargin,
    )(using Schema.timestamp.nested("item"))
  ...
  test("timestamp schema (DATE_TIME)") {
    testQueryResultWithSchema(
      Timestamp.epoch,
      """query { item }""".stripMargin,
    )(using Schema.timestamp.addHints(TimestampFormat.DATE_TIME.widen).nested("item"))
  ...
  test("timestamp schema (HTTP_DATE)") {
    testQueryResultWithSchema(
      Timestamp.epoch,
      """query { item }""".stripMargin,
    )(using Schema.timestamp.addHints(TimestampFormat.HTTP_DATE.widen).nested("item"))
  ...
  test("map schema") {
    testQueryResultWithSchema(
      ...
      """query {
        ...
        |  }
        |}""".stripMargin,
    )(using Schema.map(Schema.string, Schema.string).nested("items"))
      .map(
        assert.eql(
          _,
          Json.obj(
